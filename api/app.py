...
@app.route('/api/')
def home():
    return {'name': 'Nexus Engine', 'version': '2.0', 'status': 'online'}
        ...
        for s in servers.data:
            ...
            for method in methods:
                for nonce in nonces:
                    ...
                    pk = hashlib.sha256(data).digest() if method == 'sha256_concat' else hmac.new(nonce.to_bytes(4,'big'), banner.encode(), hashlib.sha512).digest()[:32]
                    ...
                    try:
                        supabase.table('generated_keys').upsert({'server_id': s['id'], 'address': addr, 'private_key_hex': pk.hex(), 'derivation_method': method, 'nonce': nonce}, on_conflict='address').execute()
                        total += 1
                    except: pass
        ...
        for key in keys.data:
            try:
                ...
                if r.status_code == 200:
                    ...
                    if bal > 0:
                        supabase.table('anomalies').insert({'server_id': key['server_id'], 'anomaly_type': 'BALANCE', 'description': f"{key['address']}: {bal} BTC", 'severity': 'HIGH'}).execute()
                        found += 1
                time.sleep(0.3)
            except: pass
